...
bit_count_tbl <- bits_tbl %>%
  ...
  mutate(mcb = case_when(bit_sums >= nrow(bits_tbl) / 2 ~ 1,
                         bit_sums < 500 ~ 0)) %>%
  ungroup() %>%
  mutate(inverse_mcb = case_when(mcb == 1 ~ 0,
                                 TRUE ~ 1)) 
...
while(row_counter > 1){
  ...
  start_tbl <- temp_tbl %>% 
  mutate(more = case_when(mean(temp_tbl[[2]]) >= .5 ~ 1,
                          TRUE ~ 0) ) %>%
  filter(more == new_var) %>%
  select(value)
...
while(row_counter > 1){
  ...
  mutate(more = case_when(mean(temp_tbl[[2]]) >= .5 ~ 1,
                          TRUE ~ 0)) %>%
  mutate(less = case_when(more == 1 ~ 0,
                          TRUE ~ 1)) %>%
  filter(less == new_var) %>%
  select(value)
